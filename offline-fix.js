(function(){
  ...
  const status=()=>{
    ...
    if(!n){n=document.createElement('div');n.id='offlineStatus';n.style.cssText='position:fixed;top:0;left:0;right:0;z-index:200;text-align:center;padding:5px 8px;font:700 12px Arial;background:#e8f3f0;color:#0d6658;display:none';document.body.appendChild(n)}
    n.textContent=navigator.onLine?'Online — images save on this device':'Offline — images save on this device and can be reviewed without internet';n.style.display='block';
    clearTimeout(status.t);status.t=setTimeout(()=>n.style.display='none',4000);
  };
