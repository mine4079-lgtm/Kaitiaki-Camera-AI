/* Kaitiaki Next — private Gemini vision bridge. Deploy separately, never in GitHub Pages.
...
export default {
  async fetch(request, env) {
    ...
    if (upstream.status === 429) {
      return reply({
        error: "Gemini quota or rate limit reached. Check your project limits before retrying."
      }, 429, origin);
    }

    if (!upstream.ok) {
      return reply({ error: "Gemini service returned " + upstream.status }, 502, origin);
    }
